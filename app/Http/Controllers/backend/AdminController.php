...
class AdminController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $data['mobile'] = $employee->contact;
        Admin::create($data);
        $employee->update(['is_user_created'=>1]);
    ...
    public function employeeList($branch_id)
    {
        $employees = Employee::where(['branch_id'=>$branch_id,'status'=>1,'is_user_created'=>0])->get();
    ...
    public function allAdmins(Request $request)
    {
        $query = Admin::join('roles', 'roles.id', '=', 'admins.type')
                    ->join('branches', 'branches.id', '=', 'admins.branch_id')
                    ->where('roles.is_superadmin', 0)
                    ->select('admins.id', 'admins.name', 'roles.role', 'admins.mobile', 'admins.email', 'admins.status', 'branches.title as branch_title');
